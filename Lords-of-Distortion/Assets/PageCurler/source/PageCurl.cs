...
public class PageCurl : MonoBehaviour 
{
	...
	void Update () 
    {
        ...
        {
            mat.SetFloat("_ScaleX", transform.localScale.x*10f);
            mat.SetFloat("_ScaleY", transform.localScale.z*10f);
    ...
    private Material[] GetMaterials()
    {
        if (materialRefs == null)
        {
            if (Application.isEditor)
                materialRefs = renderer.sharedMaterials;
            else
                materialRefs = renderer.materials;
        }
        return materialRefs;
    }
